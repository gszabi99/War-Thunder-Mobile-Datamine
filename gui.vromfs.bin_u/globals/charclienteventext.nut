...
from "%globalScripts/logs.nut" import log
from "%appGlobals/charClientUtils.nut" import shortKeyValue
...
function charClientEventExt(name) {
  ...
  function request(handler, params = null, context = null) {
    assert(handler in handlers, $"{name}.{handler}: Unknown handler '{handler}'")
    eventbus_send(rqEvent, { handler, params = params ?? {}, context })
  }

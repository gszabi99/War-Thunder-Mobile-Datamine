...
let function mkStage(id, nativeState, finalState) {
  let { onlyActiveStageCb, export, finalizeStage
  } = mkStageBase(id, LOGIN_STATE.AUTHORIZED, finalState)

  actions[nativeState] <- onlyActiveStageCb(finalizeStage)
...
return [
  ...
  mkStage("hangar_entered", HANGAR_ENTERED, LOGIN_STATE.HANGAR_LOADED)
]
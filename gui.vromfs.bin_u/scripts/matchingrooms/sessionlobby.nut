...
::SessionLobby.getMGameModeId <- function getMGameModeId(room = null) {
...
::SessionLobby.updateReady <- function updateReady(ready) {
  this.isReady = ready
...
::SessionLobby.updateMyState <- function updateMyState(_silent = false) {
...
::SessionLobby.setReady <- function setReady(ready, _silent = false, forceRequest = false) { //return is my info changed
...
::SessionLobby.joinRoomWithPassword <- function joinRoomWithPassword(joinRoomId, _prevPass = "", _wasEntered = false) {
  if (joinRoomId == "") {
...
::SessionLobby.getMGameMode <- @(room = null, _isCustomGameModeAllowed = true)
  gameModesRaw.value?[this.getMGameModeId(room)]

::SessionLobby.getRoomEvent <- @(room = null) this.getMGameMode(room)
...
::SessionLobby.tryJoinSession <- function tryJoinSession() {

...
import "%rGui/debugTools/applyRendererSettingsChange.nut" as applyRendererSettingsChange
...
function mkAndroidResolutions(resInfoList, presetsH, needFull) {
  ...
  foreach (v in resInfoList)
    foreach (presetH in presetsH) {
      let realW = v.w
      let realH = v.h
      let h = min(presetH, realH)
      let w = round(v.ar * h).tointeger()
      let resInfo = "".concat($"({realW}x{realH}", !needFull ? "" : $" @ {presetH}p", ")")
...
function setResolution(curResolution, resCfg, prefix = "") {
  ...
  let statusStr = " ".join([ prefix, $"Set resolution: \"{resolution}\" // {desc}" ], true)
  if (resolution == curResolution)
    return console_print(statusStr)
  setSystemConfigOption("video/resolution", resolution)
  applyRendererSettingsChange(true, statusStr)
}

function cmdChange(w, h) {
  if (!(w instanceof Integer) || w <= 0 || w > MAX_RES_SIDE || !(h instanceof Integer) || h <= 0 || h > MAX_RES_SIDE) {
...
function cmdChangeByNum(num) {
  ...
  if (!(num instanceof Integer) || num <= 0 || num > listLen) {

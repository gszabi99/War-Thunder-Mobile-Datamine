...
let icons = {
  ...
  firework_kit = "ui/gameuiskin/shop_fireworks.avif"
}
...
let getConsumablesInfo = @(goods) goods.rewards.filter(@(r) r.gType == G_ITEM)
  .sort(@(a, b) (orderByItems?[a.id] ?? 0) <=> (orderByItems?[b.id] ?? 0))
...
function mkGoodsConsumables(goods, onClick, state, animParams, addChildren) {
  ...
  let hasLimitReached = Computed(@() null != data.findvalue(function(b) {
    let { limit = 0 } = serverConfigs.get()?.allItems[b.id]
    return limit > 0 && limit <= (servProfile.get()?.items[b.id].count ?? 0)
  }))

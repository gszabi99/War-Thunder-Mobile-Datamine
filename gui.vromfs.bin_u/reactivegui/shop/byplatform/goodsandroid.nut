...
import "android.billing.googleplay" as billingModule
from "auth_wt" import registerGoogleplayPurchase, YU2_WRONG_PAYMENT, YU2_ALREADY, YU2_OK
...
from "%rGui/notifications/logEvents.nut" import logFirebaseEventWithJson, logAdjust
...
let { GP_OK, GP_NOT_INITED, GP_USER_CANCELED, GP_SERVICE_UNAVAILABLE, GP_ITEM_UNAVAILABLE,
  GP_SERVICE_TIMEOUT, GP_DEVELOPER_ERROR, SUBS_UPD_WITH_TIME_PRORATION, SUBS_UPD_DEFERRED, GP_ITEM_ALREADY_OWNED,
...
foreach(id, val in billingModule)
  if (val instanceof Integer && id.startswith("GP_"))
    statusNames[val] <- id
...
if (lastInitStatus.get() == GP_NOT_INITED)
  refreshAvailableSkus()

let updateNextRefreshTime = @(status)
  nextRefreshTime.set(status == GP_NOT_INITED || status == GP_OK ? -1 : get_time_msec() + REPEAT_ON_ERROR_MSEC)
...
function changeSubscription(subsTo, subsFrom) {
  ...
  let priceFrom = platformSubs.get()?[subsFrom]?.priceExt.price ?? 0
  let priceTo = platformSubs.get()?[subsTo]?.priceExt.price ?? 0
  logG($"Change subscription from {skuFrom} ({priceFrom}) to {skuToExt} ({priceTo})")
  upgradeSubscription(skuFrom, skuToExt, priceTo >= priceFrom ? SUBS_UPD_WITH_TIME_PRORATION : SUBS_UPD_DEFERRED)

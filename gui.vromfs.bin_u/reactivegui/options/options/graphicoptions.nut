...
let { get_maximum_frames_per_second, is_broken_grass_flag_set, is_texture_uhq_supported, should_notify_about_restart,
  get_platform_window_resolution = null, get_default_graphics_preset = @() "low"
...
let qualitiesList = (get_settings_blk()?.graphics.forceLowPreset ?? false) ? ["low"]
  ...
  : (is_android && deviceSH != null && deviceSH < 1080) ? ["low", "medium", "high"]
...
let resolutionList = (is_android && (get_user_system_info()?.physicalMemory ?? minMemory) < minMemory)
  ? ["low", "medium"]
  : ["low", "medium", "high"]

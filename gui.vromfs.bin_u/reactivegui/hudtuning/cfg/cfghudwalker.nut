...
return {
  ...
  primaryExtraGun = withActionButtonScaleCtor(AB_PRIMARY_WEAPON, mkCircleWalkerPrimaryGun(AB_PRIMARY_WEAPON),
    {
  ...
  secondaryGun = withActionButtonScaleCtor(AB_SECONDARY_WEAPON,
    mkCircleGroundSecondaryGun("ID_FIRE_WALKER_SECONDARY_GUN", AB_SECONDARY_WEAPON, "ui/gameuiskin#hud_main_weapon_fire.svg"),
    {
  ...
  specialGun = withActionButtonScaleCtor(AB_SPECIAL_WEAPON,
    mkCircleGroundSecondaryGun("ID_FIRE_WALKER_SPECIAL_GUN", AB_SPECIAL_WEAPON, "ui/gameuiskin#icon_rocket_in_progress.svg"),
    {
  ...
  machineGun = {
    ctor = @(scale) mkCircleGroundMachineGun("ID_FIRE_WALKER_MACHINE_GUN", Computed(@() actionBarItems.get()?[AB_MACHINE_GUN]), AB_MACHINE_GUN, scale)
  ...
  abExtinguisher = withActionBarButtonCtor(EII_EXTINGUISHER, WALKER,
    {
  ...
  medical = withActionBarButtonCtor(EII_MEDICALKIT, WALKER, {
  ...
  abSmokeGrenade = withAnyActionBarButtonCtor([ EII_SMOKE_GRENADE, EII_SMOKE_SCREEN ], WALKER,
    { defTransform = actionBarTransform(2) })
  abArtilleryTarget = withActionBarButtonCtor(EII_ARTILLERY_TARGET, WALKER,
    { defTransform = actionBarTransform(3) })
  abSpecialUnit2 = withActionBarButtonCtor(EII_SPECIAL_UNIT_2, WALKER,
    { defTransform = actionBarTransform(4) })
  abSpecialUnit = withActionBarButtonCtor(EII_SPECIAL_UNIT, WALKER,
  ...
  moveStick = {
    ctor = @(scale) @() {
      ...
      children = isGamepad.get() ? walkerGamepadMoveBlock(scale) : walkerMoveStick(scale)
    }
    ...
    priority = Z_ORDER.STICK
    options = [ optWalkerMoveControlType, gearDownOnStopButtonTouch ]
  }

  moveArrows = {
    ctor = @(scale) {
      key = "tank_move_stick_zone"
      children = walkerArrowsMovementBlock(scale)
    }
    defTransform = mkLBPos([0, 0])
  ...
  tacticalMap = {
    ...
    isVisibleInBattle = Computed(@() !showRadarOverMap.get() || !IsRadarHudVisible.get())
    hideForDelayed = false
  }
  ...
  radarToggle = {
    ...
    priority = Z_ORDER.BUTTON
    isVisibleInBattle = Computed(@() IsRadarHudVisible.get() && IsRadarVisible.get())
  ...
  radarHud = {
    ...
    priority = Z_ORDER.BUTTON
    isVisibleInBattle = Computed(@() IsRadarHudVisible.get() && IsRadarVisible.get() && showRadarOverMap.get())
    isVisibleInEditor = IsRadarHudVisible
  }
  ...
  compass = {
    ...
    priority = Z_ORDER.BUTTON
    isVisibleInBattle = isCompassVisible
    isVisibleInEditor = IsRadarHudVisible
  }

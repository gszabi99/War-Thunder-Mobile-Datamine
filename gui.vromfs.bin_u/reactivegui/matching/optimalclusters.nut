...
from "dagor.udp" import send, last_error
...
from "%sqstd/underscore.nut" import isEqual, getSubArray
from "%appGlobals/clientState/clientState.nut" import isInMenu
from "%appGlobals/clustersState.nut" import OPTIMAL_RTT_LIMIT_MS, OPTIMAL_RTT_LIMIT_MS_SQUAD,
  REACHABILITY_CHECK_TIMEOUT_SEC, clusterStats, optimalClusters, selClusters, unreachableHosts,
  lastReachabilityUpdateTimeMs, isWaitingManualRefresh
...
from "%appGlobals/squadState.nut" import myClustersRTT, isInSquad, isReady, squadMembers
from "%appGlobals/userstats/serverTime.nut" import gameStartServerTimeMsec
from "%rGui/matching/clusterHosts.nut" import clusterHosts
...
let hostsCfg = hardPersistWatched("hostsCfg", {})
...
function scheduleNextProbeTime(func) {
  ...
  foreach (hostInfo in hostsCfg.get()) {
...
function getClusterStats() {
  ...
  let clustersToHostsMap = getClustersToHostsMap(hostsCfg.get())
...
function tryProbeHosts() {
  if (!isProbingActive.get())
    return
  let nowMs = get_time_msec()
  hostsCfg.get().each(function(hostInfo) {
  ...
  let hostsToProbe = hostsCfg.get().filter(@(h) isNeedProbeHost(h, nowMs))
...
function onUdpPacket(evt) {
  ...
  let hostInfo = hostsCfg.get()?[host]
...
clusterHosts.subscribe(function(v) {
  hostsCfg.mutate(function(cfg) {
    foreach (ip, clustersList in v)
      if (ip in cfg)
        cfg[ip].clustersList = clustersList
      else
        cfg[ip] <- mkHost(ip, ECHO_PORT, clustersList)
    cfg.each(@(hostInfo, ip) hostInfo.isActive = ip in v)
  })
  scheduleNextProbeTime(tryProbeHosts)
})

isMatchingOnline.subscribe(function(_) {
  hostsCfg.get().each(@(hostInfo) hostInfo.__update({
    lastRequestId = 0
    lastRequestTimeMs = 0
    isReachable = false
    errors = 0

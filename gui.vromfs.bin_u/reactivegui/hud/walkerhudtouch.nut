...
let screenPositionUpdate = @(index = 1) @() index >= crosshairAmount.get()
  ? {}
  : { transform = { translate = crosshairScreenPosition.get()[index] }}
...
function mkCircleGunPosition(index) {
  ...
  let isVisible = Computed(@() index < crosshairScreenPosition.get().len()
    && crosshairScreenPosition.get()[index][0] > 0
    && crosshairScreenPosition.get()[index][1] > 0)

...
from "%rGui/legal.nut" import legalByType, FORGOT_PASSWORD_URL, REGISTER_URL
...
let termsOfServiceUrl = urlText(loc(legalByType.tos.locId), legalByType.tos.url, urlOvr)
let privacyPolicyUrl = urlText(loc(legalByType.pp.locId), legalByType.pp.url, urlOvr)

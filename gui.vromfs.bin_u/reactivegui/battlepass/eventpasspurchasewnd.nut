...
from "%rGui/battlePass/eventPassState.nut" import isEPPurchaseWndOpened, closeEPPurchaseWnd, isEpSeasonActive,
  curStage, sendEpBqEvent, purchasedEp, eventPurchasedUnlock, eventPaidRewardsUnlock, eventFreeRewardsUnlock,
  openedEventPassGoods, getEpIcon, EP_NONE, EP_COMMON, EP_VIP, getEpName, epSeasonEndTime, curEventId,
  eventPassVipLevels
...
let { defButtonHeight, COMMON } = buttonStyles
...
let header = @() {
  watch = [curEventId, epSeasonEndTime]
  size = [FLEX, gamercardHeight]
  flow = FLOW_HORIZONTAL
  valign = ALIGN_CENTER
  children = [
    backButton(closeEPPurchaseWnd)
    battlePassSeason($"events/name/{curEventId.get()}" epSeasonEndTime.get())
    { size = FLEX }
          ...
          ? textButtonPricePurchase(utf8ToUpper(loc("msgbox/btn_purchase")),
              ...
              function() {
                ...
                let eventPassRemainder = loc("eventpass/remainder", {
                  time = colorize(userlogTextColor, secondsToHoursLoc(epSeasonEndTime.get() - serverTime.get()))
                })
                if (purchList == null)

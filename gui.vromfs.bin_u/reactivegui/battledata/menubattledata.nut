...
from "%appGlobals/pServer/pServerApi.nut" import get_battle_data_jwt, get_battle_data_slots_jwt, registerHandler,
  callHandler, lastProfileKeysUpdated, get_battle_data_for_overrided_preset
...
function actualizeBattleData(unitNameOrSlots, executeAfterExt = null) {
  ...
  actualizeBattleDataByUnitsInfo(
    unitNameOrSlots instanceof Array
      ? {
          isSlots = true
...
function actualizeBattleDataIfOwn(unitNameOrSlots, executeAfterExt = null) {
  let isOwn = unitNameOrSlots instanceof Array
...
register_command(function() {
  ...
  console_print("Request battle data for unit: ", unitsInfo?.unit ?? unitsInfo?.unitList) 
}, "meta.debugCurBattleData")

register_command(function() {
  needRefreshOvrMission.set(true)
  actualizeBattleDataOvrMission("", [], "saveMenuBattleDataOvrMissionToJwt")
  console_print("Request battle data for mission with override") 
...
register_command(function() {
  if (lastResult.get()?.payload == null)
    return console_print("empty cache") 
  const fileName = "wtmBattleData.json"
  local file = io.file(fileName, "wt+")
  file.writestring(object_to_json_string(lastResult.get().payload, true))
  file.close()
  console_print($"Saved to {fileName}. Is actual ? {isBattleDataActual.get()}") 

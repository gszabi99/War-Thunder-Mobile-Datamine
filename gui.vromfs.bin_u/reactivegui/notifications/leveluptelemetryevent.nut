  ...
  foreach(campaign, level in allLevels) {
    if (level in sendEventsOn && campaign in lastLevels && lastLevels[campaign] < level) {
      ...
      if (count == sendEventsOn[level])
        sendTelemetryEvent($"levelUp_{level}_{count}")
    }
    if (sendSingleEventsOn.contains(level)) {
      sendTelemetrySavedEvent($"levelUp{level}_{campaign}", $"levelUp{campaign}{level}")
      sendTelemetrySavedEvent($"levelUp{level}_any", $"levelUpAny{campaign}{level}")
    }
  }

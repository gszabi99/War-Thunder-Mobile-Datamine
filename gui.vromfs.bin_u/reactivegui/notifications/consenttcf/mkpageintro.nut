...
let introButtons = [
  textButtonCommon(utf8ToUpper(loc("consentWnd/btns/notConsent")), @() doAnswerAllAndClose("consentMain", false))
  {size = flex()}
  textButtonPrimary(utf8ToUpper(loc("consentWnd/btns/consent")), @() doAnswerAllAndClose("consentMain", true))
]

return @() mkContent(loc("consent_tcf/intro/header"), mkIntroDesc, introButtons, doSkipClose)

from "%appGlobals/unitConst.nut" import *

let sameLoc = @(locId, scList) scList.map(@(value) { locId, value })
let cLoc = @(locId, value) { locId, value }

let shortcutsByUnitTypes = {
  [SHIP] = {
    shortcuts = [
      "ID_FLIGHTMENU", "ID_ZOOM_TOGGLE", "ID_COMBINATION",
      cLoc("flightmenu/btnStats", "ID_MPSTATSCREEN"),
      cLoc("hotkeys/primaryGun", "ID_SHIP_WEAPON_PRIMARY"),
      cLoc("hotkeys/secondaryGun", "ID_SHIP_WEAPON_SECONDARY"),
      cLoc("hotkeys/extraGun1", "ID_SHIP_WEAPON_EXTRA_GUN_1"),
      cLoc("hotkeys/extraGun2", "ID_SHIP_WEAPON_EXTRA_GUN_2"),
      cLoc("hotkeys/extraGun3", "ID_SHIP_WEAPON_EXTRA_GUN_3"),
      cLoc("hotkeys/extraGun4", "ID_SHIP_WEAPON_EXTRA_GUN_4"),
      cLoc("HUD/TXT_IRCM", "ID_IRCM_SWITCH_SHIP"),
      cLoc("hotkeys/ID_SHIP_WEAPON_DEPTH_CHARGE", "ID_SHIP_WEAPON_MORTAR"),
      cLoc("hotkeys/ID_SHIP_ACTION_BAR_ITEM_11", "ID_SHIP_ACTION_BAR_ITEM_40"),
      "ID_SHIP_WEAPON_TORPEDOES", "ID_SHIP_SMOKE_SCREEN_GENERATOR", "ID_WTM_LAUNCH_AIRCRAFT",
      "ID_SHIP_WEAPON_MINE", "ID_SHIP_ACTION_BAR_ITEM_11", "ID_SHOW_HERO_MODULES",
      "ID_SHIP_STRATEGY_MODE_TOGGLE", "ID_SHIP_WEAPON_ROCKETS", "ID_SHIP_WEAPON_ROCKETS_SECONDARY",
      
      "ID_SHIP_ROCKET_ASW",
      "ID_SHIP_TOGGLE_CIWS", "ID_SHIP_WEAPON_ROCKETS_TERTIARY",
      cLoc("hotkeys/ID_SHOW_VOICE_MESSAGE_LIST", "ID_VOICE_MSG"), "ID_CHANGE_ZOOM_DEC", "ID_CHANGE_ZOOM_INC",
      "ID_ELECTRONIC_WARFARE"
    ]
      .extend(sameLoc("controls/unit_control", ["ship_steering_rangeMin", "ship_steering_rangeMax",
        "ship_main_engine_rangeMax", "ship_main_engine_rangeMin"]))
    axes = sameLoc("hotkeys/ID_PLANE_MOUSE_AIM_HEADER", ["ship_mouse_aim_x", "ship_mouse_aim_y"])
  },
  [SUBMARINE] = {
    shortcuts = [
      "ID_FLIGHTMENU", "ID_ZOOM_TOGGLE", "ID_COMBINATION",
      cLoc("flightmenu/btnStats", "ID_MPSTATSCREEN"),
      "ID_SUBMARINE_WEAPON_TORPEDOES", "ID_DIVING_LOCK", "ID_SHIP_WEAPON_MINE",
      "ID_SUBMARINE_WEAPON_ROCKETS", "ID_SUBMARINE_ACOUSTIC_COUNTERMEASURES",
      "ID_SUBMARINE_ACTION_BAR_ITEM_11", "ID_SHOW_HERO_MODULES",
      cLoc("hotkeys/ID_SHIP_ACTION_BAR_ITEM_11", "ID_SUBMARINE_ACTION_BAR_ITEM_40"),
      cLoc("HUD/TXT_IRCM", "ID_SUBMARINE_ACOUSTIC_DECOY"),
      cLoc("hotkeys/ID_SHOW_VOICE_MESSAGE_LIST", "ID_VOICE_MSG"), "ID_CHANGE_ZOOM_DEC", "ID_CHANGE_ZOOM_INC",
    ]
      .extend(sameLoc("controls/unit_control", ["ship_steering_rangeMin", "ship_steering_rangeMax",
          "submarine_main_engine_rangeMax", "submarine_main_engine_rangeMin"]),
        sameLoc("controls/depth_change", ["submarine_depth_inc", "submarine_depth_dec"]))
    axes = sameLoc("hotkeys/ID_PLANE_MOUSE_AIM_HEADER", ["submarine_mouse_aim_x", "submarine_mouse_aim_y"])
  },
  [AIR] = {
    shortcuts = [
      "ID_FLIGHTMENU", "ID_ZOOM_TOGGLE", "ID_COMBINATION",
      cLoc("flightmenu/btnStats", "ID_MPSTATSCREEN"),
      "ID_BOMBS", "ID_ROCKETS", "ID_FIRE_COURSE_GUNS", "ID_FIRE_CANNONS", "ID_FIRE_MGUNS",
      "ID_LOCK_TARGET", "ID_CAMERA_NEUTRAL", "ID_CAMERA_VIEW_BACK", "ID_SHOW_HERO_MODULES",
      cLoc("hotkeys/ID_SHIP_WEAPON_TORPEDOES", "ID_WTM_AIRCRAFT_TORPEDOES"),
      cLoc("hotkeys/ID_WTM_LAUNCH_AIRCRAFT/aircraft", "ID_WTM_LAUNCH_AIRCRAFT"),
      cLoc("hotkeys/ID_TOGGLE_VIEW", "ID_CAMERA_VIEW_STICK"),
      "ID_CTRL_PIE_STICK", "ID_TACTICAL_MAP",
      cLoc("hotkeys/ID_SHOW_VOICE_MESSAGE_LIST", "ID_VOICE_MSG"), "ID_CHANGE_ZOOM_DEC", "ID_CHANGE_ZOOM_INC",
      cLoc("flare/name", "ID_COUNTERMEASURES_FLARES"),
    ]
    axes = sameLoc("controls/unit_control", ["throttle_axis", "ailerons", "elevator", "rudder"])
      .extend(sameLoc("controls/help/movement_direction_control", ["mouse_aim_x", "mouse_aim_y"]),
        sameLoc("controls/help/turret_aiming_control", ["turret_x", "turret_y"]))
  },
  [TANK] = {
    shortcuts = [
      "ID_FLIGHTMENU", "ID_ZOOM_TOGGLE", "ID_COMBINATION",
      cLoc("flightmenu/btnStats", "ID_MPSTATSCREEN"),
      "ID_FIRE_GM", "ID_FIRE_GM_SECONDARY_GUN", "ID_FIRE_GM_SPECIAL_GUN", "ID_FIRE_GM_MACHINE_GUN",
      "ID_ACTION_BAR_ITEM_5", "ID_ACTION_BAR_ITEM_7", "ID_ACTION_BAR_ITEM_9", "ID_ACTION_BAR_ITEM_6",
      "ID_ACTION_BAR_ITEM_10", "ID_SMOKE_SCREEN_GENERATOR", "ID_SMOKE_SCREEN", "ID_SHOW_HERO_MODULES",
      "ID_CAMERA_NEUTRAL", "ID_TOGGLE_TARGET_TRACKING", "ID_NEXT_BULLET_TYPE", "ID_TACTICAL_MAP",
      cLoc("hotkeys/ID_SHIP_ACTION_BAR_ITEM_11", "ID_ACTION_BAR_ITEM_11"),
      cLoc("hotkeys/ID_SHOW_VOICE_MESSAGE_LIST", "ID_VOICE_MSG"), "ID_CHANGE_ZOOM_DEC", "ID_CHANGE_ZOOM_INC",
    ]
    axes = sameLoc("controls/unit_control", ["gm_throttle", "gm_steering"])
      .extend(sameLoc("hotkeys/ID_PLANE_MOUSE_AIM_HEADER", ["gm_mouse_aim_x", "gm_mouse_aim_y"]))
  },
  [WALKER] = {
    shortcuts = [
      "ID_FLIGHTMENU", "ID_ZOOM_TOGGLE", "ID_COMBINATION",
      cLoc("flightmenu/btnStats", "ID_MPSTATSCREEN"),
      "walker_throttle_rangeMax", "walker_throttle_rangeMin",
      "ID_FIRE_GM", "ID_FIRE_GM_SECONDARY_GUN", "ID_FIRE_GM_SPECIAL_GUN", "ID_FIRE_GM_MACHINE_GUN",
      "ID_ACTION_BAR_ITEM_5", "ID_ACTION_BAR_ITEM_7", "ID_ACTION_BAR_ITEM_9", "ID_ACTION_BAR_ITEM_6",
      "ID_ACTION_BAR_ITEM_10", "ID_SMOKE_SCREEN_GENERATOR", "ID_SMOKE_SCREEN", "ID_SHOW_HERO_MODULES",
      "ID_CAMERA_NEUTRAL", "ID_TOGGLE_TARGET_TRACKING", "ID_NEXT_BULLET_TYPE", "ID_TACTICAL_MAP",
      cLoc("hotkeys/ID_SHIP_ACTION_BAR_ITEM_11", "ID_ACTION_BAR_ITEM_11"),
      cLoc("hotkeys/ID_SHOW_VOICE_MESSAGE_LIST", "ID_VOICE_MSG"), "ID_CHANGE_ZOOM_DEC", "ID_CHANGE_ZOOM_INC",
      "ID_FIRE_WALKER_SECONDARY_GUN", "ID_WALKER_ACTION_BAR_ITEM_10",
    ]
    axes = sameLoc("controls/unit_control", ["walker_throttle", "walker_steering"])
      .extend(sameLoc("hotkeys/ID_PLANE_MOUSE_AIM_HEADER", ["walker_mouse_aim_x", "walker_mouse_aim_y"]))
  }
}

return {
  shortcutsByUnitTypes
  pages = unitTypeOrder.filter(@(ut) ut in shortcutsByUnitTypes)
}